...
GAS_TRACE_IDENTITY_DOMAIN = "blue-contracts-gas-trace/1.0"
GAS_TRACE_REQUIRED_IDENTITY_FIELDS = (
    "sequence",
    "namespace",
    "counter",
    "quantity",
...
GAS_TRACE_OPTIONAL_IDENTITY_FIELDS = (
    "documentId",
    "scopePath",
...
GAS_TRACE_DIAGNOSTIC_FIELDS = frozenset(("reason",))
GAS_TRACE_FIELDS = frozenset(
    GAS_TRACE_REQUIRED_IDENTITY_FIELDS
    + GAS_TRACE_OPTIONAL_IDENTITY_FIELDS
) | GAS_TRACE_DIAGNOSTIC_FIELDS


def gas_trace_identity_projection(
    entries: Iterable[Mapping[str, Any]],
) -> list[dict[str, Any]]:
    """Project observable gas entries onto their identity-bearing fields.

    ``reason`` remains part of the observable trace, but its free-form
    diagnostic wording is deliberately absent from the identity projection.
    ``logicalPath`` remains identity-bearing because Contracts 1.0 uses it as
    exact semantic charge attribution rather than as a logging source path.
    """
    result: list[dict[str, Any]] = []
    for ordinal, entry in enumerate(entries):
        unknown = set(entry) - GAS_TRACE_FIELDS
        if unknown:
            raise ValueError(
                f"gas trace entry {ordinal} has unknown fields: "
                f"{sorted(unknown)!r}"
            )
        missing = set(GAS_TRACE_REQUIRED_IDENTITY_FIELDS) - set(entry)
        if missing:
            raise ValueError(
                f"gas trace entry {ordinal} lacks identity fields: "
                f"{sorted(missing)!r}"
            )
...
def gas_trace_identity(entries: Iterable[Mapping[str, Any]]) -> str:
    """Return the Contracts 1.0 identity of an admitted gas trace."""
    envelope = {
        "domain": GAS_TRACE_IDENTITY_DOMAIN,
        "value": gas_trace_identity_projection(entries),
    }
...
def finalize_cyclic_component(
    ...
    *,
    ...
    attribution_defaults: Mapping[str, Any] | None = None,
) -> int:
    base_context = dict(attribution_defaults or {})
    base_context.update({
