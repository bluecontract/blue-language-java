...
final class NestedValueReferenceIdentityTest {
    ...
    @Test
    void shouldPreserveUntypedChildIdentityWhenTheFieldDeclaresACustomType() {
        // The retained child is deliberately untyped; only Holder contributes Terms.
        BasicNodeProvider provider = new BasicNodeProvider();
        try (BlueLanguage language = BlueLanguage.builder().nodeProvider(provider).build()) {
            String termsTypeId = store(language, provider,
                    source(language, "name: Terms\ntitle:\n  type: Text\n"));
            String holderTypeId = store(language, provider,
                    new Node().name("Holder").properties("terms",
                            new Node().type(reference(termsTypeId))));
            Node terms = source(language, "title:\n  type: Text\n  value: Coffee\n");
            Node retained = language.identity().canonicalIdentityInput(terms);
            org.junit.jupiter.api.Assertions.assertNull(retained.getType(),
                    "The child must not acquire its enclosing field's type before storage");
            String termsId = store(language, provider, terms);
            Node inline = new Node().type(reference(holderTypeId)).properties("terms", terms.clone());
            Node referenced = new Node().type(reference(holderTypeId)).properties("terms", reference(termsId));
            Node inlinePreprocessed = language.preprocessing().preprocess(inline);
            Node referencedPreprocessed = language.preprocessing().preprocess(referenced);

            // Establish representation equivalence independently of Source reconstruction.
            assertEquals(language.identity().directBlueId(inlinePreprocessed),
                    language.identity().directBlueId(referencedPreprocessed));
            assertEquals(NodeWireForm.get(language.graph().expand(inlinePreprocessed)),
                    NodeWireForm.get(language.graph().expand(referencedPreprocessed)));
            String inlineId = language.identity().sourceDocumentBlueId(inline);
            String referencedId = language.identity().sourceDocumentBlueId(referenced);
            String minimized = language.codec().write(language.resolution().minimize(inline), BlueFormat.JSON);
            String reloadedId;
            try (BlueLanguage reader = BlueLanguage.builder().nodeProvider(provider).build()) {
                reloadedId = reader.identity().sourceDocumentBlueId(
                        reader.codec().parseSource(minimized, BlueFormat.JSON));
            }
            org.junit.jupiter.api.Assertions.assertAll(
                    () -> assertEquals(inlineId, referencedId, () ->
                            "Untyped retained child " + termsId + "; inline canonical: "
                                    + NodeWireForm.get(language.identity().canonicalIdentityInput(inline))
                                    + "; referenced canonical: "
                                    + NodeWireForm.get(language.identity().canonicalIdentityInput(referenced))),
                    () -> assertEquals(inlineId, reloadedId, "Cold minimized reload must retain Source identity"));
        }
    }
