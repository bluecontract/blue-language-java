...
def build_vector_coverage(ordinary: list[Path], closure: list[Path]) -> dict[str, Any]:
    ...
    result = {
        ...
        "ordinaryVectorCount": len([v for v in mapping if not v.startswith("C-CLO-")]),
        "closureVectorCount": len([v for v in mapping if v.startswith("C-CLO-")]),
...
def build_release_manifest(fixture_manifest: dict[str, Any], oracle_manifest: dict[str, Any], language_root: Path | None) -> dict[str, Any]:
    ...
    release: dict[str, Any] = {
        ...
        "sourceArchiveBaseline": {
            "expectedSha256": "8e50f397c842f2adbd2ffd0d28b78da3489a2efe33cd6e8775fa28954f863831",
...
def package_files() -> list[Path]:
    ...
    for path in ROOT.rglob("*"):
        ...
        if rel in EXCLUDED_PACKAGE_PATHS:
            continue
        if "__pycache__" in path.parts or "build/classes" in rel:
            continue
        result.append(path)
...
def write_checksum_manifest() -> None:
    lines: list[str] = []
    for path in sorted(ROOT.rglob("*"), key=lambda p: p.relative_to(ROOT).as_posix() if p.exists() else ""):
        if not path.is_file():
            continue
        rel = path.relative_to(ROOT).as_posix()
        if rel in {"MANIFEST.sha256", "validation-output.json"}:
            continue
        if "__pycache__" in path.parts or "build/classes" in rel:
            continue
