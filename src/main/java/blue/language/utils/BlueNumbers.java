...
public final class BlueNumbers {
    ...
    public static boolean isExactBinary64Multiple(Object value, BigDecimal multipleOf) {
        ...
        if (multipleDouble == 0.0d || !Double.isFinite(multipleDouble)) {
            throw new IllegalArgumentException("Double multipleOf must be finite and non-zero.");
        }
        Binary64Rational valueRational = Binary64Rational.fromDouble(valueDouble);
        Binary64Rational multipleRational = Binary64Rational.fromDouble(multipleDouble);
    ...
    private static double toDouble(Object value) {
        ...
        if (value instanceof BigDecimal) {
            ...
            throw new IllegalArgumentException("Double value must be numeric: " + value);
        }
    ...
    private static final class Binary64Rational {
        ...
        private Binary64Rational(BigInteger numerator, BigInteger denominator) {
        ...
        private static Binary64Rational fromDouble(double value) {
            ...
            if (value == 0.0d) {
                return new Binary64Rational(BigInteger.ZERO, BigInteger.ONE);
            }
            ...
            if (exponent >= 0) {
                return new Binary64Rational(significand.shiftLeft(exponent), BigInteger.ONE);
            }
            return new Binary64Rational(significand, BigInteger.ONE.shiftLeft(-exponent));
        }

        private boolean dividedByIsInteger(Binary64Rational divisor) {
            if (divisor.numerator.signum() == 0) {
                throw new IllegalArgumentException("Division by zero rational.");
            }
            BigInteger quotientNumerator = numerator.multiply(divisor.denominator);
            BigInteger quotientDenominator = denominator.multiply(divisor.numerator).abs();
            return quotientNumerator.remainder(quotientDenominator).signum() == 0;
        }
    }
}
